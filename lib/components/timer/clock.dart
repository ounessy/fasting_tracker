...
class TimerState extends State<ClockUI> with SingleTickerProviderStateMixin {
  ...
  /// valueNotifier to define duruation of the fast
  final ValueNotifier<int> duration = ValueNotifier(1);
  ...
  void initState() {
    ...
    controller.setDelay(widget.delay);
  ...
  @override
  void didUpdateWidget(ClockUI oldWidget) {
  ...
  void _startTimer([bool useDelay = true]) {
    if (useDelay && !controller.wasActive) {
  ...
  String getPassedTimeText() {
    Duration duration = controller.getPasserTime();
    if (widget.progressTextFormatter == null) {
      return "${duration.inHours}:${duration.inMinutes % 60}:${(duration.inSeconds % 60).toString().padLeft(2, "0")}";
    }
    return widget.progressTextFormatter!(duration);
